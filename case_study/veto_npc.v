...
Local Notation " '<<' c1 , c2 , c3 '>>' " := (cons _ c1 _ (cons _ c2 _ (cons _ c3 _ (nil _)))).

Definition zero_vote := << 0, 0, 0 >>.
Definition add_votes {n:nat} v1 v2 := @map2 int int int (fun x y => x + y) n v1 v2. 
Definition fold_votes vs := List.fold_left add_votes vs zero_vote. 
...
|manipulate_ : forall votes, p_wins (add_votes base_votes votes) ->
                        manipulate base_votes. 

Fixpoint build_veto_a ks :=
  match ks with
    |k::ks' => add_votes << k*2, 0, k*2 >> (build_veto_a ks')
    |List.nil => zero_vote
  end. 

Fixpoint build_veto_b ks :=
  match ks with
    |k::ks' => add_votes << k*2, k*2, 0 >> (build_veto_b ks')
    |List.nil => zero_vote
  end. 
...
Theorem sum_veto_a : forall l k, sum l = k -> build_veto_a l = << k*2, 0, k*2 >>. 
Proof.
  induction l; intros. 
